...
std::list<boost::shared_ptr<NodeEntry> > RTreeHelper::search(Node *subtreeRoot, const boost::shared_ptr<Rectangle> &query)
{
    ...
    {
        for(it =entries.begin(); it!=entries.end(); ++it) //the entries are the type "NodeEntry" -- base class. "LeafEntry" and "NonLeafEntry" are its derived class.
        {
            if((*it)->getMBR()->intersects(*query))
            {
                boost::shared_ptr<Rectangle> r = (*it)->getMBR();
                // std::cout << r->getLower()[0] << "\t" << r->getLower()[1] << "\t"
                //           << r->getUpper()[0] << "\t" << r->getUpper()[1] << "\n";
                // std::cout << (query)->getLower()[0] << "\t" << (query)->getLower()[1] << "\t"
                //           << (query)->getUpper()[0] << "\t" << (query)->getUpper()[1] << "\n";
                result.push_back(*it);
            }
        }
    }
