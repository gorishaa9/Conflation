...
public:
    ...
    vector<JoinResult> processRankJoin(map<SKey, RVal> relations, char* ck, char* count){
        // exit( EXIT_FAILURE );
        ...
        start_indices                 = rj_utils.initializeStartIndices( n-1 ) ;
                                        rj_utils.calculateLengths( segments[0] );
        ...
        while( output.size() < k || int(tau*1000000) > int(min_output_score*1000000) ){
            ...
            ir_tuple ir = rj_utils.getNextJoinTuple( i );
            ...
            // double la_score = ( group_score_weights[ group_number ] * t_bounds[ group_number ] );
            ...
            // prev_scores[ group_number ] = la_score;
